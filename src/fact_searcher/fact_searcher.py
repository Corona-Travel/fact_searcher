...
def fact_parser(soup: BeautifulSoup) -> str:
    ...
    if (
        result.find("</b>") + 4 == parenthesis_start
        or result.find("</i>") + 4 == parenthesis_start
    ):
...
async def extract_coords(link: str, session: HTMLSession) -> list:
    ...
    elements = coordinates.html.xpath(
        "//table//th[text()='Latitude']/following-sibling::td"
    )
...
async def search_facts(name: str, session: HTMLSession) -> list:
    ...
    try:
        soup = BeautifulSoup(
            wiki.html.xpath(
                "//div[@class='mw-parser-output']/p[not(@class='mw-empty-elt')]",
                first=True,
            ).html,
            "html.parser",
        )
...
async def main(city: str) -> list:
    try:
        ...
        sights = session.get(
            "".join(city_search.html.xpath(f"//a[text()='{city}']")[0].absolute_links)
        )
